...
process_date <- as.Date("2020-8-12")

nagoya_city_covid19 <- read_csv("nagoya_city_covid19(20200812).csv", 
...
symptom$Age0 <- sprintf("%d<br>(%3.1f)", symptom$Age0, round(100 * symptom$Age0 / sum(symptom$Age0)))
symptom$Age20 <- sprintf("%d<br>(%3.1f)", symptom$Age20, round(100 * symptom$Age20 / sum(symptom$Age20)))
symptom$Age40 <- sprintf("%d<br>(%3.1f)", symptom$Age40, round(100 * symptom$Age40 / sum(symptom$Age40)))
symptom$Age60 <- sprintf("%d<br>(%3.1f)", symptom$Age60, round(100 * symptom$Age60 / sum(symptom$Age60)))
symptom$Age80 <- sprintf("%d<br>(%3.1f)", symptom$Age80, round(100 * symptom$Age80 / sum(symptom$Age80)))
...
* 他機関から陽性事例のみの報告があったものも含まれるため、陽性率算出の際の分母としてはご利用できません。
